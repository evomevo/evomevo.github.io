...
const colors = Array(21).fill("#8000FF");

const settings = {
  startTime: new Date().getTime(), // This can be in the future
  duration: 1000, // Total time for all dots to realign at the starting point
...
const draw = () => { // Definitely not optimized
  ...
  arcs.forEach((arc, index) => { // Draw arcs
    ...
    pen.globalAlpha = determineOpacity(currentTime, arc.lastImpactTime, 0.4, 1, 1000);
